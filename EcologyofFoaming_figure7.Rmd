...
title: Ecology of foam establishment during anaerobic digestion of wastewater sludge
  following process disturbance - Figures 7 and S12 (metabolome)
output:
  html_document:
    df_print: paged
editor_options:
...
taxa <-  data.frame(t(taxa))
taxa <-  data.frame(t(taxa))
metabolome <- data.frame(read.csv("./data/020524-metabolimicsresults_LOGresponseFULLChris.csv")) 
# remove first three rows
metabolome <- metabolome[c(4:nrow(metabolome)),] 
metabolome <- metabolome %>% remove_rownames %>% column_to_rownames("Samples")
metabolome <- data.frame(t(metabolome)) %>% 
...
p3 <- phyloseq::plot_ordination(psobject,
    ...
    annotate(geom = 'text', label = paste("Composition of 221 metabolites \n PCA, log(response ratio)"),
             x = -Inf, y = -Inf, hjust = -0.05, vjust = -0.5, size = 3) +
          ylim(xdim) + xlim(xdim) + ggtitle("Log(response ratios) of all peaks") +
           theme(plot.title = element_text(size = 10),
                 ) +
  geom_text(aes(label = sample_data(psobject)$Date), nudge_y = -0.12, size = 2, show.legend = FALSE)

p3

ggsave(plot = p3, "./Figures/Sup_metabomlome_allpeaks.png", height=10, width=13, units='cm', dpi=600)
```
