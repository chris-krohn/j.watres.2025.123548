...
function(pf, ps.phylo, metadatacolumn,
         layout = "circular",
         branch.length = "none",
         factorvector = NULL,
         offset.text = 5,
         pwidth = 0.05) {
  metadatacolumn <- dplyr::enquo(metadatacolumn)
  ...
  melt_simple <- psmelt(ps.phylo) %>%
    ...
    dplyr::select(OTU, val=Abundance, !!metadatacolumn) %>% unique() #unquote metadatacolumn
  ...
  taxa.df.genus <- phyloseq::tax_table(ps.phylo)@.Data %>%
    as.data.frame %>%
    rownames_to_column("OTUID") %>%
    dplyr::select(OTUID, Genus) %>%
    column_to_rownames("OTUID")
  phyla.list <- split(rownames(taxa.df.genus),
                      taxa.df.genus$Genus,
                      drop = TRUE)
  ## adding that phyla list to the tree
  tree.df <-  phyloseq::phy_tree(pf$tree)
  ### add list grouping to tree and call it Phylum - GEnus
  ...
  p <-  ggtree::ggtree(ggtree_gps,
                       ...
                       layout = layout,
                       branch.length = "none",
                       key_glyph = "rect") +    # change this for a rectangular legend symbol
  ...
  #  add  the heatmap showing presence/abundances for each tip
p <-  p + ggnewscale::new_scale_fill() +
    geom_fruit(data=melt_simple, geom=geom_tile,
               mapping=aes(y=OTU, x=!!metadatacolumn, alpha=val, fill=!!metadatacolumn),
               color = "lightgrey", offset = 0.04, size = 0.001,
               pwidth = pwidth) +
    scale_alpha_continuous(range=c(0.01, 1),
                           guide=guide_legend(keywidth = 0.3,
                                             keyheight = 0.3, order=1)) +
  viridis::scale_fill_viridis(discrete = TRUE)  +
    guides(alpha = FALSE)  # choose as per your liking
...
#                      breaks = ordervec)
  ...
  for(i in factorvector) {
    p <- p +  geom_cladelabel(node= (df %>% dplyr::filter(Phylofactor == i))$node,
                              ...
                              offset.text= offset.text, offset = -0.25,
                              hjust='center')
  }
  return(p)
}
